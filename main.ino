...
// Effect states
uint16_t rainbowFirstPixelHueA = 0, rainbowFirstPixelHueB = 0;
unsigned long meteorLastUpdateA = 0, meteorLastUpdateB = 0;
int meteorPosA = 0, meteorPosB = 0;
byte heatA[MAX_LEDS];
byte heatB[MAX_LEDS];
...
void effect_rainbow(Adafruit_NeoPixel &strip, const StripSettings &s, int startLed, int endLed, uint16_t &hue);
void effect_meteor(Adafruit_NeoPixel &strip, const StripSettings &s, int startLed, int endLed, unsigned long &lastUpdate, int &meteorPos);
void effect_fire(Adafruit_NeoPixel &strip, const StripSettings &s, int startLed, int endLed, byte heat[]);
...
void saveConfiguration() {
  ...
  auto saveStrip = [&](JsonObject obj, const StripSettings &s) {
    obj["type"] = s.type;
    obj["length"] = s.length;
    obj["density"] = s.density;
    obj["offset"] = s.offset;
    obj["mode"] = s.mode;
    obj["followWidth"] = s.followWidth;
    obj["wipeSpeed"] = s.wipeSpeed;
    obj["onTime"] = s.onTime;
    obj["effect"] = s.effect;
    obj["color"] = s.colorHex;
    obj["temp"] = s.colorTemp;
    obj["brightness"] = s.brightness;
  };
  if (strip1_settings.enabled) {
    JsonObject s1 = doc.createNestedObject("strip1");
    saveStrip(s1, strip1_settings);
  }
...
void loadConfiguration() {
  ...
  if (SPIFFS.exists("/config.json")) {
    ...
    if (doc.containsKey("strip1")) loadStrip(doc["strip1"].as<JsonObjectConst>(), strip1_settings);
    else strip1_settings.enabled = false;
...
void handleGet() {
  ...
  if (strip1_settings.enabled) {
    JsonObject s1 = doc.createNestedObject("strip1");
    s1["type"] = strip1_settings.type;
    s1["length"] = strip1_settings.length;
    s1["density"] = strip1_settings.density;
    s1["offset"] = strip1_settings.offset;
    s1["mode"] = strip1_settings.mode;
    s1["effect"] = strip1_settings.effect;
    s1["color"] = strip1_settings.colorHex;
    s1["brightness"] = strip1_settings.brightness;
  }
  if (strip2_settings.enabled) {
    JsonObject s2 = doc.createNestedObject("strip2");
    s2["type"] = strip2_settings.type;
    s2["length"] = strip2_settings.length;
    s2["density"] = strip2_settings.density;
    s2["offset"] = strip2_settings.offset;
    s2["mode"] = strip2_settings.mode;
    s2["effect"] = strip2_settings.effect;
    s2["color"] = strip2_settings.colorHex;
    s2["brightness"] = strip2_settings.brightness;
  }
  String out;
...
void sendStateToClients() {
  ...
  // Presence is true if motion was detected within the configured onTime
  bool presence1 = (millis() - lastMotionTimeB) < (unsigned long)(strip1_settings.onTime * 1000UL);
  bool presence2 = (millis() - lastMotionTimeA) < (unsigned long)(strip2_settings.onTime * 1000UL);
...
void applyEffectsAndUpdate() {
  static int lastWrittenA = -1, lastWrittenB = -1;
  static int prevStartLedA = -1, prevEndLedA = -1;
  ...
  if (mainMode == "on") {
    ...
    if (strip1_settings.enabled) {
      ...
      if (isAnimated || lastWrittenB != strip1_settings.numLeds) {
        if (strip1_settings.effect == "rainbow") effect_rainbow(stripB, strip1_settings, 0, strip1_settings.numLeds, rainbowFirstPixelHueB);
        else if (strip1_settings.effect == "meteor") effect_meteor(stripB, strip1_settings, 0, strip1_settings.numLeds, meteorLastUpdateB, meteorPosB);
        else if (strip1_settings.effect == "fire") effect_fire(stripB, strip1_settings, 0, strip1_settings.numLeds, heatB);
    ...
    // --- Strip A (2) ---
    if (strip2_settings.enabled) {
      bool isAnimated = (strip2_settings.effect != "solid");
      if (isAnimated || lastWrittenA != strip2_settings.numLeds) {
        if (strip2_settings.effect == "rainbow") effect_rainbow(stripA, strip2_settings, 0, strip2_settings.numLeds, rainbowFirstPixelHueA);
        else if (strip2_settings.effect == "meteor") effect_meteor(stripA, strip2_settings, 0, strip2_settings.numLeds, meteorLastUpdateA, meteorPosA);
        else if (strip2_settings.effect == "fire") effect_fire(stripA, strip2_settings, 0, strip2_settings.numLeds, heatA);
        else stripA.fill(strip2_settings.color);
        updatedA = true;
      }
  ...
  if (strip1_settings.enabled) {
    if (manualOverride) {
      if (led1_on) {  // led1_on corresponds to strip B (strip 1)
    ...
    } else {  // AUTOMATIC SENSOR MODE FOR STRIP B
      int startLed = 0, endLed = 0;
      bool motionDetected = smoothedDistanceB > 0;
      if (strip1_settings.mode == "fill") {
        int ledsToLight = motionDetected ? map((int)smoothedDistanceB - strip1_settings.offset, 0, strip1_settings.length * 100, 0, strip1_settings.numLeds) : 0;
        endLed = constrain(ledsToLight, 0, strip1_settings.numLeds);
      } else if (strip1_settings.mode == "depart") {
        endLed = strip1_settings.numLeds;
        startLed = motionDetected ? constrain(map((int)smoothedDistanceB - strip1_settings.offset, 0, strip1_settings.length * 100, 0, strip1_settings.numLeds), 0, strip1_settings.numLeds) : 0;
        ...
        if (motionDetected) {
          ...
          startLed = constrain(centerLed - (strip1_settings.followWidth / 2), 0, strip1_settings.numLeds);
          endLed = constrain(centerLed + (strip1_settings.followWidth / 2), 0, strip1_settings.numLeds);
        }
      }

      if (startLed != prevStartLedB || endLed != prevEndLedB || strip1_settings.effect != "solid") {
        updatedB = true;
        if (strip1_settings.effect == "rainbow") effect_rainbow(stripB, strip1_settings, startLed, endLed, rainbowFirstPixelHueB);
        else if (strip1_settings.effect == "meteor") effect_meteor(stripB, strip1_settings, startLed, endLed, meteorLastUpdateB, meteorPosB);
        else if (strip1_settings.effect == "fire") effect_fire(stripB, strip1_settings, startLed, endLed, heatB);
        else {
  ...
  if (strip2_settings.enabled) {
    if (manualOverride) {
      if (led2_on) {  // led2_on corresponds to strip A (strip 2)
    ...
    } else {  // AUTOMATIC SENSOR MODE FOR STRIP A
      int startLed = 0, endLed = 0;
      bool motionDetected = smoothedDistanceA > 0;
      if (strip2_settings.mode == "fill") {
        int ledsToLight = motionDetected ? map((int)smoothedDistanceA - strip2_settings.offset, 0, strip2_settings.length * 100, 0, strip2_settings.numLeds) : 0;
        endLed = constrain(ledsToLight, 0, strip2_settings.numLeds);
      } else if (strip2_settings.mode == "depart") {
        endLed = strip2_settings.numLeds;
        startLed = motionDetected ? constrain(map((int)smoothedDistanceA - strip2_settings.offset, 0, strip2_settings.length * 100, 0, strip2_settings.numLeds), 0, strip2_settings.numLeds) : 0;
      } else if (strip2_settings.mode == "follow") {
        if (motionDetected) {
          int centerLed = map((int)smoothedDistanceA - strip2_settings.offset, 0, strip2_settings.length * 100, 0, strip2_settings.numLeds);
          startLed = constrain(centerLed - (strip2_settings.followWidth / 2), 0, strip2_settings.numLeds);
          endLed = constrain(centerLed + (strip2_settings.followWidth / 2), 0, strip2_settings.numLeds);
        }
      }

      if (startLed != prevStartLedA || endLed != prevEndLedA || strip2_settings.effect != "solid") {
        updatedA = true;
        if (strip2_settings.effect == "rainbow") effect_rainbow(stripA, strip2_settings, startLed, endLed, rainbowFirstPixelHueA);
        else if (strip2_settings.effect == "meteor") effect_meteor(stripA, strip2_settings, startLed, endLed, meteorLastUpdateA, meteorPosA);
        else if (strip2_settings.effect == "fire") effect_fire(stripA, strip2_settings, startLed, endLed, heatA);
        else {
          for (int i = 0; i < strip2_settings.numLeds; i++) {
            stripA.setPixelColor(i, (i >= startLed && i < endLed) ? strip2_settings.color : 0);
          }
        }
      }
      prevStartLedA = startLed;
      prevEndLedA = endLed;
      lastWrittenA = endLed;
    }
  } else {
...
void setup() {
  ...
  server.onNotFound([]() {
    if (!handleFileRead(server.uri())) handleFileRead("/index.html");
  });
  server.begin();
...
void loop() {
  ...
  // handle incoming serial from sensor A
  while (SensorSerialA.available() > 0) {
    char c = SensorSerialA.read();
    if (c == '\n') {
      if (lineBufferA.startsWith("Range")) {
        String ds = lineBufferA.substring(5);
        int d = ds.toInt();
        if (d > 0) {
          currentDistanceA = d;
          lastMotionTimeA = millis();
        }
      }
      lineBufferA = "";
    } else if (c >= 32) lineBufferA += c;
  }
  ...
  // update smoothed distances (simple lerp)
  unsigned long now = millis();
  float alpha = 0.25f;  // smoothing factor (0..1) - prilagodljivo

  // Sensor A
  if ((now - lastMotionTimeA) > (unsigned long)(strip2_settings.onTime * 1000UL)) {
    smoothedDistanceA = smoothedDistanceA * 0.90f;
    if (smoothedDistanceA < 0.5f) smoothedDistanceA = 0;
  } else {
    smoothedDistanceA = smoothedDistanceA + alpha * ((float)currentDistanceA - smoothedDistanceA);
  }
